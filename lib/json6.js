	...
	var word = WORD_POS_RESET,
		...
		inQueue = {
			...
			push(node) {
				...
				if( !this.last ) this.first = recover; // uncovered; this is more of a program sequence error/programmer error that won't arrise in correct usage.
				this.last = recover;
			},
			shift() {
				...
				if( !this.first ) this.last = null; // uncovered; this is more of a program sequence error/programmer error that won't arrise in correct usage.
				result.next = this.saved; this.saved = result;
				return result.node;
			},
			unshift(node) {
