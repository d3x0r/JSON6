...
const SUPPORT_LEAD_ZERO_OCTAL = false;
		...
		_write(msg,complete_at_end) {
				...
				while( retval == 0 && ( n < buf.length ) )
				{
					...
					if( cInt == start_c )//( cInt == 34/*'"'*/ ) || ( cInt == 39/*'\''*/ ) || ( cInt == 96/*'`'*/ ) )
					{
						...
						if( stringOct ) {
							...
							else if( cInt >= 97/*'a'*/ && cInt <= 102/*'f'*/ ) hex_char += ( cInt - 96 ) + 10;
							else {
			...
			while( status && ( input = inQueue.shift() ) ) {
				...
				while( !completed && status && ( n < buf.length ) )
				{
					...
					if( n >= 0x10000 ) { str += buf.charAt(n); n++; }
					...
					case 44/*','*/:
						...
						{
							status = false;
							throwError( "bad context; fault while parsing;" );// fault
						}
						negative = false;
						break;
