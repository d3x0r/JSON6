				...
				while( (_n = input.n),(( input.n < input.buf.length ) && (c = input.buf.charAt(input.n++)) ) )
				{
					  ...
					  )
					{
						...
						if( !fromHex && val.string == '0' ) {
							...
							status = false;
							throw new Error( `fault while parsing number; '${c}' unexpected at ${input.n}  ${pos.line}:${pos.col}` );
							break;
						}
					}
				...
				while( !completed && status && ( input.n < input.buf.length ) && ( c = input.buf.charAt(input.n++) ) )
				{
					...
					if( comment ) {
						if( comment == 1 ) {
							...
							if( cInt != 47/*'/'*/ ) {
								throw new Error( `Fault while parsing; unexpected '${c}' at ${input.n} ${pos.line}:${pos.col}` );
								status = false;
							}
							else comment = 2;
							continue;
					...
					case 123/*'{'*/:
						if( word == WORD_POS_FIELD || word == WORD_POS_AFTER_FIELD || ( parse_context == CONTEXT_OBJECT_FIELD && word == WORD_POS_RESET ) ) {
							throw new Error( `Fault while parsing; getting field name unexpected '${c}' at ${input.n} ${pos.line}:${pos.col}` );
							status = false;
							break;
						}
						{
					...
					//
						...
						default:
							if( ( cInt >= 48/*'0'*/ && cInt <= 57/*'9'*/ ) || ( cInt == 43/*'+'*/ ) || ( cInt == 46/*'.'*/ ) )
							{
								fromHex = false;
								val.string = c;
								collectNumber();
