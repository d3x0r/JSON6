...
let _DEBUG_LL = false;
let _DEBUG_PARSING = false;
let _DEBUG_PARSING_STACK = false;
let SUPPORT_LEAD_ZERO_OCTAL = true;
...
var JSON6 = typeof exports === 'object'
    ? exports
...
JSON6.supportLeadingOctals = function (support) {
    SUPPORT_LEAD_ZERO_OCTAL = support;
};
        ...
        _write(msg,complete_at_end) {
            ...
            function gatherString( start_c ) {
                ...
                while( retval == 0 && ( n < buf.length ) ) {
                    ...
                    if( cInt == start_c ) {//( cInt == 34/*'"'*/ ) || ( cInt == 39/*'\''*/ ) || ( cInt == 96/*'`'*/ ) )
                        ...
                        case 2029: // PS (paragraph separator)
                        ...
                        if( cr_escaped ) {
                            cr_escaped = false;
                            // '\r\n'  does not emit either character?
                            /*
                         * this really can't happen. ( \\ \r (and now no escape)
                         *    if( cInt == 10/*'\n' * / ) {
                         *        pos.line++;
                          *        pos.col = 1;
                         *        stringEscape = false;
                        ...
                        /*    }*/
                            //continue;  // this character should still be emitted... just a position thing.
                        }
                        val.string += str;
                    }
                }
            ...
            while( status && ( input = inQueue.shift() ) ) {
                ...
                while( !completed && status && ( n < buf.length ) ) {
                    ...
                    log('_DEBUG_LL', "processing: ", cInt, str, pos, comment, parse_context, word );
                    pos.col++;
                    if( comment ) {
                        if( comment == 1 ) {
