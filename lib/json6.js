	...
	return {
		...
		_write(msg,complete_at_end) {
			...
			function throwError( leader, c ) {
				throw new Error( `${leader} '${c}' unexpected at ${input.n-1} (near '${input.buf.substr(input.n>4?(input.n-4):0,input.n>4?3:(input.n-1))}[${c}]${input.buf.substr(input.n, 10)}') [${pos.line}:${pos.col}]`)			
			}
			...
			function arrayPush() {
				...
				case VALUE_NUMBER:
					element_array.push( Number( val.string ) * (val.negative?-1:1) );
					break;
				case VALUE_STRING:
			...
			function objectPush() {
				switch( val.value_type ){
				case VALUE_NUMBER:
					elements[val.name] = Number( val.string ) * (val.negative?-1:1);
					break;
				case VALUE_STRING:
				...
				while( (_n = input.n),(( input.n < input.buf.length ) && (c = input.buf.charAt(input.n++)) ) )
				{
					...
					if( cInt >= 48/*'0'*/ && cInt <= 57/*'9'*/ )
					{
						...
						if( val.string.length == 0 || ( exponent && !exponent_sign && !exponent_digit ) ) {
							...
							status = false;
							throwError( "fault while parsing number;", c );
							break;
						}
					} else if( cInt == 46/*'.'*/ ) {
				...
				while( !completed && status && ( input.n < input.buf.length ) && ( c = input.buf.charAt(input.n++) ) )
				{
					...
					if( comment ) {
						if( comment == 1 ) {
							...
							if( cInt != 47/*'/'*/ ) {
								throwError( "fault while parsing;", c );
								status = false;
							}
							else comment = 2;
							continue;
					...
					case 123/*'{'*/:
						if( word == WORD_POS_FIELD || word == WORD_POS_AFTER_FIELD || ( parse_context == CONTEXT_OBJECT_FIELD && word == WORD_POS_RESET ) ) {
							throwError( "fault while parsing; getting field name unexpected ", c );
							status = false;
							break;
						}
						{
					...
					case 91/*'['*/:
						if( parse_context == CONTEXT_OBJECT_FIELD || word == WORD_POS_FIELD || word == WORD_POS_AFTER_FIELD ) {
							throw new Error( `Fault while parsing; while getting field name unexpected '${c}' at ${input.n} ${pos.line}:${pos.col}` );
							status = false;
							break;
						}
						{
					...
					case 93/*']'*/:
						...
						{
							throw new Error( `bad context ${context}; fault while parsing; '${c}' unexpected at ${input.n}  ${pos.line}:${pos.col}`);// fault
							status = false;
						}
						negative = false;
						break;
						...
						if( parse_context == CONTEXT_OBJECT_FIELD ) {
							...
							case 39://'\'':
								if( word == WORD_POS_RESET ) {
									...
									throw new Error( `fault while parsing; quote not at start of field name at ${input.n-1} state:${word} (near ${input.buf.substr(input.n>4?(input.n-4):0,input.n>4?3:(input.n-1))}[${c}]${input.buf.substr(input.n, 10)}) [${pos.line}:${pos.col}]`)
								}
							...
							default:
								if( word == WORD_POS_AFTER_FIELD ) {
									status = false;
									throw new Error( `fault while parsing; character unexpected at ${input.n-1} state:${word} (near ${input.buf.substr(input.n>4?(input.n-4):0,input.n>4?3:(input.n-1))}[${c}]${input.buf.substr(input.n, 10)}) [${pos.line}:${pos.col}]`)
								}
					...
					//  catch characters for true/false/null/undefined which are values outside of quotes
						case 116://'t':
							...
							else { status = false; throwError( "fault parsing", c ); }// fault
							break;
						case 114://'r':
